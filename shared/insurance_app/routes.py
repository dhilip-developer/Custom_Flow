...
from app.models.schemas import AgentResult, BatchResultItem, BatchResponse, ExpandedBillOfLading
...
async def get_agent1_status():
    ...
    from app.engine.detector import is_ai_enabled
...
async def upload_file(file: UploadFile = File(...)):
    ...
    try:
        ...
        if name_lower.endswith(".pdf") or file.content_type == "application/pdf":
            from app.engine.pdf_extractor import extract_data_from_pdf
...
async def extract_bol(
    ...
    """
    from app.engine.bol_extractor import extract_expanded_bol
    try:
...
async def get_agent2_status():
    """Returns whether the AI engine is active or in local fallback mode."""
    from app.engine.bol_extractor import is_ai_enabled
    return {"ai_enabled": is_ai_enabled()}
